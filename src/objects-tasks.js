...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    if (queue[i] - 25 > acc) return false;
    acc += 25;
  }
  return true;
}
...
function group(array, keySelector, valueSelector) {
  ...
  array.forEach((geoEntity) => {
    const key = keySelector(geoEntity);
    const val = map.get(key) || [];
    val.push(valueSelector(geoEntity));
    map.set(key, val);
  });
  return map;
}
...
const cssSelectorBuilder = {
  ...
  element(value) {
    if (this.res.find((e) => e.type === 'element'))
      throw new Error(
    ...
    newObj.res = [...this.res];
    newObj.res.push({ type: 'element', value: `${value}` });
    return newObj;
  },

  id(value) {
    if (
      !['class', 'attr', 'pseudoClass', 'pseudoElement'].every((e) => {
        return this.res.find((el) => el.type === e) === undefined;
      })
    )
      throw new Error(
        'Selector parts should be arranged in the following order: element, id, class, attribute, pseudo-class, pseudo-element'
      );
    if (this.res.find((e) => e.type === 'id'))
      throw new Error(
        'Element, id and pseudo-element should not occur more than one time inside the selector'
      );
    ...
    const newObj = Object.create(cssSelectorBuilder);
    newObj.res = [...this.res];
    newObj.res.push({ type: 'id', value: `#${value}` });
    return newObj;
  },

  class(value) {
    if (
      !['attr', 'pseudoClass', 'pseudoElement'].every((e) => {
        return this.res.find((el) => el.type === e) === undefined;
      })
    )
      throw new Error(
    ...
    const newObj = Object.create(cssSelectorBuilder);
    newObj.res = [...this.res];
    newObj.res.push({ type: 'class', value: `.${value}` });
    return newObj;
  },

  attr(value) {
    if (
      !['pseudoClass', 'pseudoElement'].every((e) => {
        return this.res.find((el) => el.type === e) === undefined;
      })
    )
      throw new Error(
    ...
    const newObj = Object.create(cssSelectorBuilder);
    newObj.res = [...this.res];
    newObj.res.push({ type: 'attr', value: `[${value}]` });
    return newObj;
  },

  pseudoClass(value) {
    if (
      !['pseudoElement'].every((e) => {
        return this.res.find((el) => el.type === e) === undefined;
      })
    )
      throw new Error(
    ...
    const newObj = Object.create(cssSelectorBuilder);
    newObj.res = [...this.res];
    newObj.res.push({ type: 'pseudoClass', value: `:${value}` });
    return newObj;
  },

  pseudoElement(value) {
    if (this.res.find((e) => e.type === 'pseudoElement'))
      throw new Error(
        'Element, id and pseudo-element should not occur more than one time inside the selector'
      );
    ...
    const newObj = Object.create(cssSelectorBuilder);
    newObj.res = [...this.res];
    newObj.res.push({ type: 'pseudoElement', value: `::${value}` });
    return newObj;
  },
